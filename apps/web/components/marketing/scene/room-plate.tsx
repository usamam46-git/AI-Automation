...
/**
 * The plate is referenced by PATH, not by a static `import` — deliberately.
 *
 * A static import is the usual Next idiom and gives intrinsic dimensions plus a
 * generated `blurDataURL`. It also **breaks CI**, and the reason is worth
 * writing down because the failure is invisible on a developer machine:
 *
 *  - `*.jpg` is declared by `next/image-types/global`, which is referenced from
 *    `next-env.d.ts`;
 *  - `next-env.d.ts` is **gitignored** and is generated by `next build`/`dev`;
 *  - CI runs `tsc --noEmit` *before* `npm run build`, on a clean checkout.
 *
 * So the declaration simply does not exist when the type-check runs, and
 * `Cannot find module '@/public/desk-room.jpg'` is the result — while locally it
 * type-checks fine, because a dev server has generated the file at some point.
 * (In Next 16 `next-env.d.ts` also imports `./.next/types/routes.d.ts`, so a
 * pre-build `tsc` is on thin ice generally.)
 *
 * Nothing is lost here. `fill` means the intrinsic dimensions are unused, and
 * `unoptimized` means the build-time pipeline is skipped anyway — a static
 * import bought only the blur placeholder, and the backdrop gradient beneath
 * this layer already paints first and *is* the placeholder.
 */
const PLATE_SRC = "/desk-room.jpg";
