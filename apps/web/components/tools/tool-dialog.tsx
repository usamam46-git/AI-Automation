...
import { TOOL_NAME_PATTERN, type Tool, type ToolType, toolsApi } from "@/lib/api";
...
function ToolDialogForm({ tool, onOpenChange }: { tool: Tool | null; onOpenChange: (open: boolean) => void }) {
  ...
  const configComplete = toolType === "http_request" ? url.trim().length > 0 : action.length > 0;
  ...
  function buildConfig(): Record<string, unknown> {
    ...
    for (const key of ["url", "method", "headers", "timeout_seconds", "action"]) delete preserved[key];

    return toolType === "http_request"
      ? { ...preserved, url: url.trim(), method, headers, timeout_seconds: timeoutSeconds }
      : { ...preserved, action };
  }
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (editing) {
        return toolsApi.update(tool.id, { name: trimmedName, description: description.trim() || null, config, is_mutating: isMutating });
      }
...
export function ToolDialog({ tool, open, onOpenChange }: { tool: Tool | null; open: boolean; onOpenChange: (open: boolean) => void }) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent>{open ? <ToolDialogForm key={tool?.id ?? "new-tool"} tool={tool} onOpenChange={onOpenChange} /> : null}</DialogContent>
    </Dialog>
  );
}
